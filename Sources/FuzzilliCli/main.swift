...
if args["-h"] != nil || args["--help"] != nil || args.numPositionalArguments != 1 {
    print(
        ...
        Options:
            ...
            --evaluator=name             : The program evaluator to use. Available evaluators: "coverage" (default), "distance".
                                           "distance" enables directed greybox fuzzing using distance-based guidance.
            --corpus=name                : The corpus scheduler to use. Available schedulers: "basic" (default), "markov", "aflgo".
                                           "aflgo" implements AFLGo-style directed greybox fuzzing with distance-based energy assignment.
            ...
            --defaultMutationBudget=n    : Maximum mutation count for a sample in AFLGo corpus. Samples closer to targets receive
                                           proportionally more mutations (default: 2 * minMutationsPerSample). Only used with --corpus=aflgo.
            --guidedByTargetSpecificDistance : Enable target-specific distance guidance for seed selection. Only used with --corpus=aflgo.
            ...
            --setProcessArgs=args        : Override the profile's default process arguments with custom arguments. If multiple arguments are passed,
                                           they should be separated by a comma. When this is set, --argumentRandomization will be ignored.
            --setProcessEnv=env          : Pass custom environment variables to the JS engine. If multiple variables are passed,
                                           they should be separated by a comma, e.g., --setProcessEnv=VAR1=VAL1,VAR2=VAL2.
...
let evaluatorName = args["--evaluator"] ?? "coverage"
...
// This is for AFLGoCorpus, defining the default mutation budget for a sample
let defaultMutationBudget = args.int(for: "--defaultMutationBudget") ?? minMutationsPerSample
...
let setProcessArgs = args["--setProcessArgs"]
let setProcessEnv = args["--setProcessEnv"]
...
let guidedByTargetSpecificDistance = args.has("--guidedByTargetSpecificDistance")
...
let validEvaluators = ["coverage", "distance"]
guard validEvaluators.contains(evaluatorName) else {
    configError("--evaluator must be one of \(validEvaluators)")
}

let validCorpora = ["basic", "markov", "aflgo"]
...
if corpusName == "markov" && evaluatorName != "coverage" {
    configError("Markov corpus is only compatible with the coverage evaluator")
}

if corpusName == "aflgo" && evaluatorName != "distance" {
    configError("AFLGo corpus requires the distance evaluator")
}

if corpusName != "aflgo" && args.int(for: "--defaultMutationBudget") != nil {
    configError("The defaultMutationBudget setting is only compatible with the aflgo corpus")
}

if corpusName == "aflgo" && staticCorpus {
    configError("AFLGo corpus is not compatible with --staticCorpus")
}
...
if setProcessArgs != nil && argumentRandomization {
    configError("--setProcessArgs is not compatible with --argumentRandomization.")
}
...
let additionalArgs =
    additionalArguments.isEmpty ? [] : additionalArguments.split(separator: ",").map(String.init)
let jsShellArguments: [String]
if let customProcessArgs = setProcessArgs {
    // Use user-specified arguments, split by comma
    let userProcessArgs: [String] = customProcessArgs.split(separator: ",").map(String.init)
    jsShellArguments = userProcessArgs + additionalArgs
} else {
    // Use profile's default arguments with optional randomization
    jsShellArguments = profile.processArgs(argumentRandomization) + additionalArgs
}
...
func makeFuzzer(with configuration: Configuration) -> Fuzzer {
    ...
    var processEnv = profile.processEnv
    if let customProcessEnv = setProcessEnv {
        for entry in customProcessEnv.split(separator: ",") {
            let parts = entry.split(separator: "=", maxSplits: 1)
            guard parts.count == 2 else {
                configError(
                    "Invalid environment variable entry: \(entry). Must be of the form KEY=VALUE")
            }
    ...
    let runner = REPRL(
        ...
        processEnvironment: processEnv, maxExecsBeforeRespawn: profile.maxExecsBeforeRespawn
    )
    ...
    let evaluator: ProgramEvaluator
    switch evaluatorName {
    case "coverage":
        evaluator = ProgramCoverageEvaluator(runner: runner)
    case "distance":
        evaluator = ProgramDistanceEvaluator(runner: runner)
    default:
        logger.fatal("Invalid evaluator name provided")
    }
    ...
    case "markov":
        // Markov corpus requires ProgramCoverageEvaluator
        guard let covEvaluator = evaluator as? ProgramCoverageEvaluator else {
            logger.fatal("Markov corpus requires coverage evaluator")
        }
        corpus = MarkovCorpus(covEvaluator: covEvaluator, dropoutRate: markovDropoutRate)
    case "aflgo":
        guard evaluator is ProgramDistanceEvaluator else {
            logger.fatal("AFLGo corpus requires distance evaluator")
        }
        corpus = AFLGoCorpus(
            minSize: minCorpusSize, maxSize: maxCorpusSize,
            defaultMutationBudget: defaultMutationBudget,
            guidedByTargetSpecificDistance: guidedByTargetSpecificDistance)
    default:
...
// Record the fuzzing session start time (shared across all instances)
let fuzzingStartTime = UInt64(Date().timeIntervalSince1970 * 1000)
logger.info("Fuzzing start time: \(fuzzingStartTime)")
...
let mainConfig = Configuration(
    ...
    fuzzingStartTime: fuzzingStartTime)
